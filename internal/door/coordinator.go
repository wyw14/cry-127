...
func (c *Coordinator) Release(operationID string, now time.Time) (State, interlock.Decision, error) {
	...
	gasProof.BackfillClosing = c.backfillClosing(operationID)
...
func (c *Coordinator) backfillClosing(operationID string) bool {
	state, ok := c.atmosphere.Current(operationID)
	if !ok || state.ActiveLine == "" {
		return false
	}
	return !state.IsolatedLines[state.ActiveLine]
}
