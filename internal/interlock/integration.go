...
func (i *Integration) AtmospherePermit(operation model.Operation, gasProof model.GasProof, vacuumProof model.VacuumProof, now time.Time) (Decision, error) {
	checks := map[string]bool{
		"vacuum proof is not current": vacuumProof.OperationID == operation.ID && vacuumProof.SealRevision == operation.SealRevision && vacuumProof.Durable,
		"gas path is not ready":       atmosphere.Permit(gasProof, vacuumProof, operation.SealRevision) == nil,
	}
